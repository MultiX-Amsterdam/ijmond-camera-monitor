CREATE OR REPLACE FUNCTION weekly_champion()
RETURNS VOID
LANGUAGE plpgsql AS
$$
DECLARE
    weekly_champion_id integer := 2;
    max_score_user_id integer;
    weekNow int := EXTRACT(WEEK FROM CURRENT_DATE);
...
BEGIN
    ...
    FROM weekly_score
    WHERE week = weekNow AND year = yearNow
    ...
    IF max_score_user_id IS NOT NULL THEN
        ...
        WHERE user_id = max_score_user_id AND achievement_id = weekly_champion_id;
        ...
        ELSE
            ...
            VALUES (max_score_user_id, weekly_champion_id, 1);
        END IF;
        ...
        WHERE user_id = max_score_user_id AND achievement_id = weekly_champion_id AND date = today;

        -- If not found, then insert the new record
        IF NOT FOUND THEN
            INSERT INTO achievement_day (user_id, achievement_id, date)
            VALUES (max_score_user_id, weekly_champion_id, today);
        END IF;
