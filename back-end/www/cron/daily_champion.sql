CREATE OR REPLACE FUNCTION daily_champion()
RETURNS VOID
LANGUAGE plpgsql AS
$$
DECLARE
    daily_champion_id integer := 1;
...
BEGIN
    ...
    IF max_score_user_id IS NOT NULL THEN
        ...
        WHERE user_id = max_score_user_id AND achievement_id = daily_champion_id;
        ...
        ELSE
            ...
            VALUES (max_score_user_id, daily_champion_id, 1);
        END IF;
        ...
        WHERE user_id = max_score_user_id AND achievement_id = daily_champion_id AND date = today;

        -- If not found, then insert the new record
        IF NOT FOUND THEN
            INSERT INTO achievement_day (user_id, achievement_id, date)
            VALUES (max_score_user_id, daily_champion_id, today);
        END IF;
